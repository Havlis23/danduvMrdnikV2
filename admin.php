...
viewMenu(
    ["admin" => "Správa aut", "admin_users" => "Správa uživatelů"],
    0,
    1,
    $_SESSION["loguser"]["fullname"]
);
?>
<body>
...
<section>
  ...
  <?php
  $query =
      "select vozy.id as id, modely.model as model, rok_vyroby, barvy.barva as barva, najeto, cena from vozy join barvy on vozy.barva = barvy.id join modely on vozy.model=modely.id";
  try {
      $q = $db->prepare($query);
      $q->execute();
